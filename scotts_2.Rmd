...
cld_1 %>%
  ...
  mutate(time = "Fall") |> 
  ggplot(aes(x = fct_reorder(population, rate), y = rate, color = time)) +
  ...
  geom_text_repel(aes(label = round(rate,0), 
                    ...
                    seed = 455) +
  geom_text(aes(x = population, y = upper.CL + 35, label = .group, color = time),
            inherit.aes = FALSE) +
  scale_color_manual(values = "#004586") +
  geom_curve(data = crn, mapping = aes(x = 2.05, y = 48, xend = 2.2, yend = 200),
             arrow = arrow(length = unit(0.07, "inch")), 
             size = 0.4, curvature = -0.3, color = "#004586", inherit.aes = FALSE) +
  geom_text(data = crn, mapping = aes(x = 2.2, y = 220), label = "Fall", 
