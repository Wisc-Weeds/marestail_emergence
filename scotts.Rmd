...
readxl::read_excel("anova.xlsx", sheet = "SB") %>% 
  ...
  mutate(trt = case_when(
    ...
    pop == "L" & treat == "ir" ~ "Lin in irrigated",
    pop == "NP" & treat == "ir" ~ "Npl in irrigated",
    pop == "SB" & treat == "ir" ~ "Scb in irrigated",
    TRUE ~ NA_character_
  )) %>% 
...
newdata2 <- data.frame(trt =c("Lin in irrigated"), newdata)
newdata3 <- data.frame(trt =c("Npl in fallow"), newdata)
newdata4 <- data.frame(trt =c("Npl in irrigated"), newdata)
newdata5 <- data.frame(trt =c("Scb in fallow"), newdata)
newdata6 <- data.frame(trt =c("Scb in irrigated"), newdata)
